...
def calculate_3nn_acc(Bi_DataX, Bi_DataY, select, base=5):
    if not isinstance(select, (list, np.ndarray)):
        raise ValueError("参数select必须是一个列表或者数组")
    if max(select) >= Bi_DataX.shape[1] or min(select) < 0:
        raise ValueError("参数select中包含了非法的特征索引值")
...
#分数据是均匀分布还是聚类分布
    ...
    def SFS(self, train_time):
        # 初始化一个空的特征集合
        selected_features = np.zeros(self.featuresNum, dtype=bool)

        # 设定停止条件，比如当特征数量达到3个时停止
        stop_condition = int(0.4 * self.featuresNum)
        while np.sum(selected_features) < stop_condition:
            # 初始化当前最高分数和对应的特征索引
            max_score = -1
            best_feature = None

            # 遍历剩余的特征
            for feature_index in range(self.featuresNum):
                if not selected_features[feature_index]:
                    # 创建当前的特征子集
                    selected_features[feature_index] = True  # 先临时将特征选入集合
                    select = np.where(selected_features)[0]
                    fitness_value = calculate_3nn_acc(self.B_Train_X[1][train_time], self.B_Train_Y[1][train_time],
                                                      select)
                    selected_features[feature_index] = False  # 恢复未选择状态
                    # 如果当前特征子集的性能更好，则更新最高分数和最佳特征
                    if fitness_value > max_score:
                        max_score = fitness_value
                        best_feature = feature_index

            # 将最佳特征添加到已选特征集合中
            selected_features[best_feature] = True
        select = np.where(selected_features)[0]
        self.B_Gbest_Zi[1] = selected_features
        self.B_Gbest_Value[1] = calculate_3nn_acc(self.B_Train_X[1][train_time], self.B_Train_Y[1][train_time],
                                                  select)


    def SBS(self, train_time):
        # 初始化一个空的特征集合
        selected_features = np.ones(featuresNum, dtype=bool)

        # 设定停止条件，比如当特征数量达到0.4个时停止
        stop_condition = int(0.4 * self.featuresNum)
        while np.sum(selected_features) > stop_condition:
            # 初始化当前最高分数和对应的特征索引
            accuracies = []
            ...
            for feature in range(featuresNum):
                if selected_features[feature]:
                    # 创建副本，删除当前特征
                    mask = np.ones(featuresNum, dtype=bool)
                    mask[feature] = 0
                    select = np.argwhere(mask == 1)
                    select = select.flatten()
                    # 计算准确率
                    accuracy = calculate_3nn_acc(self.B_Train_X[2][train_time], self.B_Train_Y[2][train_time],
                                                      select)
            ...
            selected_features[best_index] = False

        self.B_Gbest_Zi[2] = selected_features
        select = np.argwhere(selected_features == 1)
        select = select.flatten()
        self.B_Gbest_Value[2] = calculate_3nn_acc(self.B_Train_X[2][train_time], self.B_Train_Y[2][train_time],
                                                      select)

            # # 记录每次迭代的最佳特征和准确率
            # best_selected_features.append((best_index, best_accuracy))
    ...
    def FPSO_Train(self, Train_times=5):
        ...
        print(mean_acc_with_A)
        mean_acc_with = np.mean(mean_acc_with_A)
        return mean_acc_with
...
#保存数据结果的
    def Algo(self):
        ...
        each_time_acc_no_A = np.zeros(selectTimes)
        # each_time_acc_with_A = np.zeros((self.selectTimes, self.B_Num))
        for select_time in range(selectTimes):

            each_time_acc_no_A[select_time] = self.FPSO_Train()

            # 将结果保存
        print(np.mean(each_time_acc_no_A))
# wine = load_wine()
